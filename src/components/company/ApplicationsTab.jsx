...
const ApplicationsTab = ({ isDark, isMobile }) => {
  ...
  if (loading) {
    return (
      <div className={`p-4 md:p-6 rounded-xl md:rounded-2xl backdrop-blur-xl border transition-all duration-500 ${
        isDark
        ...
        <div className="text-center py-12">
          ...
          <p className={isDark ? "text-gray-400" : "text-gray-600"}>Loading applications...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-start gap-4">
                <div className="w-20 h-20 rounded-2xl bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center text-white font-bold text-2xl">
                  {getInitials(selectedApplication.user?.name || selectedApplication.user?.username)}
                </div>
              ...
              <div className={`p-4 rounded-xl ${isDark ? "bg-slate-800/50" : "bg-gray-50"}`}>
                ...
                <p className={isDark ? "text-gray-300" : "text-gray-700"}>
                  {new Date(selectedApplication.createdAt || selectedApplication.appliedAt).toLocaleDateString('en-US', {
                    weekday: 'long',
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
